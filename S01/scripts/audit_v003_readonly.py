...
def evidence_exists(description: str) -> bool:
    # Evidence strings may include multiple filenames and candidate labels (T027/G320).
    names=re.findall(r'[A-Za-z0-9_./-]+\.(?:png|jpg|jpx|json)',description)
    return bool(names) and all((R/name if '/' in name else R/'history/interrupted_reread/qa'/name).is_file() for name in names)
...
def run():
    ...
    check('source_evidence_exists_for_every_change',all(evidence_exists(c['evidence']) and c['source_sha256']==EXPECTED_SOURCE and not c['canonical_Arabic_changed'] for c in changes))
    ...
    check('apparatus_ledgers_synchronized',notes==load('ledgers/v003_adjudications.json') and [n for n in notes if n['status']!='RESOLVED_RETAINED']==load('ledgers/unresolved_readings.json'))
